...
app.put('/org', auth, allow('manager'), (req,res)=>{
  ...
  saveDB(db); res.json(db.org);
});

// ====== USERS ======
app.get('/users', auth, (req,res)=>{
  const me=getUser(req.user.id); let rows=[];
  if(['admin','hr'].includes(me.role)) rows = db.users.map(stripPass);
  else if(me.role==='manager') rows = db.users.filter(u=>u.id===me.id || u.managerId===me.id).map(stripPass);
  else rows = db.users.filter(u=>u.id===me.id).map(stripPass);
  res.json(rows);
});
app.post('/users', auth, allow('manager'), (req,res)=>{
  const me=getUser(req.user.id); if(!['admin','hr'].includes(me.role)) return res.status(403).json({error:'forbidden'});
  const u=req.body||{}; if(!u.name||!u.email||!u.role) return res.status(400).json({error:'missing_fields'});
  const id=u.id||uid();
  db.users.push({ id, name:u.name, email:u.email, role:u.role, managerId:u.managerId||null, contract:u.contract||'1.0', hoursMonthly:u.hoursMonthly||null, startDate:u.startDate||null, pass:u.pass||'1', vacationDays: u.vacationDays ?? 26 });
  saveDB(db); res.json({id});
});
app.put('/users/:id', auth, allow('manager'), (req,res)=>{
  const me=getUser(req.user.id); if(!['admin','hr'].includes(me.role)) return res.status(403).json({error:'forbidden'});
  const id=req.params.id; const u=db.users.find(x=>x.id===id); if(!u) return res.status(404).json({error:'not_found'});
  Object.assign(u, cleanUndefined({ name:req.body.name, email:req.body.email, role:req.body.role, managerId:req.body.managerId, contract:req.body.contract, hoursMonthly:req.body.hoursMonthly, startDate:req.body.startDate, pass:req.body.pass, vacationDays:req.body.vacationDays }));
...
app.get('/times', auth, (req,res)=>{
  ...
  const month=req.query.month; const like=(d)=> month? String(d||'').startsWith(month) : true;
  let rows=[];
  if(['admin','hr'].includes(me.role)) rows = month? db.times.filter(t=>like(t.date)) : db.times;
  else if(me.role==='manager') rows = db.times.filter(t=> like(t.date) && (t.userId===me.id || isManagerOf(me.id,t.userId)) );
  else rows = db.times.filter(t=> t.userId===me.id && like(t.date));
  res.json(rows);
});
app.post('/times', auth, (req,res)=>{
  const me=getUser(req.user.id); const t=req.body||{};
...
app.put('/times/:id', auth, (req,res)=>{
  ...
  const row=db.times.find(x=>x.id===id); if(!row) return res.status(404).json({error:'not_found'});
  const can = ['admin','hr'].includes(me.role) || row.userId===me.id || (me.role==='manager' && isManagerOf(me.id,row.userId));
  if(!can) return res.status(403).json({error:'forbidden'});
  Object.assign(row, cleanUndefined({ date:req.body.date, start:req.body.start, end:req.body.end, break:req.body.break, project:req.body.project, note:req.body.note, status:req.body.status }));
  saveDB(db); res.json({ok:true});
});
...
app.get('/leaves', auth, (req,res)=>{
  const me=getUser(req.user.id); let rows=[];
  if(['admin','hr'].includes(me.role)) rows = db.leaves;
  else if(me.role==='manager') rows = db.leaves.filter(l=> l.userId===me.id || isManagerOf(me.id,l.userId) );
  else rows = db.leaves.filter(l=> l.userId===me.id );
  res.json(rows);
});
app.post('/leaves', auth, (req,res)=>{
  const me=getUser(req.user.id); const l=req.body||{};
...
app.put('/leaves/:id', auth, (req,res)=>{
  const me=getUser(req.user.id); const id=req.params.id;
  const row=db.leaves.find(x=>x.id===id); if(!row) return res.status(404).json({error:'not_found'});
  const can = ['admin','hr'].includes(me.role) || (me.role==='manager' && isManagerOf(me.id,row.userId)) || row.userId===me.id;
  if(!can) return res.status(403).json({error:'forbidden'});
  Object.assign(row, cleanUndefined({ type:req.body.type, from:req.body.from, to:req.body.to, comment:req.body.comment, status:req.body.status }));
  saveDB(db); res.json({ok:true});
});
